...
class Game{
    ...
    sendGameData(data){
        this.io.to(this.roomCode).emit('bg-colour-change',data);
    }
    ...
    sendDataToPlayer(playerName, data){
        socket.broadcast.to(getUserByNameAndCode(playerName, this.roomCode).id).emit('updateGame', data);
    } 


    
    //Impliment the following methods in subclass version of your game, as they will be used by the server. Not all need to actually do anything.
